...
-(NSString *)processResponceWithResponce:(NSURLResponse *)responce andError:(NSError *)error{
    ...
    if(httpResponce.statusCode>=400&&httpResponce.statusCode<=499){ return [NSString stringWithFormat:@"Client error: %ld",(long)httpResponce.statusCode]; }
    if(httpResponce.statusCode>=500&&httpResponce.statusCode<=599){ return [NSString stringWithFormat:@"Server error: %ld",(long)httpResponce.statusCode]; }
